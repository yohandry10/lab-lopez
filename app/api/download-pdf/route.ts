/* app/api/download-pdf/route.ts
   ...
   · Requiere que añadas  ORION_API_TOKEN  en tus variables de entorno.
*/

import { NextRequest, NextResponse } from "next/server"

const ORION_API_TOKEN = process.env.ORION_API_TOKEN  // ← añade esto en .env.local
...
export async function GET(req: NextRequest) {
  ...
  if (!ORION_API_TOKEN) {
    return NextResponse.json({ error: "Falta ORION_API_TOKEN en variables de entorno" }, { status: 500 })
  }
  ...
  try {
    const orionRes = await fetch(orionURL, {
      headers: {
        ...
        Authorization: `Bearer ${ORION_API_TOKEN}`,
      },
