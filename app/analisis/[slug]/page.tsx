...
export default function AnalysisPage({ params }: PageProps) {
  ...
  const foundArticle = analysisData.find((a) => a.slug === params.slug)
  ...
  const heroIcons = useMemo(() => {
    return foundArticle.heroIcons?.map((iconName) => {
      switch (iconName) {
        case "Baby":
          return <Icons.Baby key={iconName} className="w-6 h-6" />
        case "Dna":
          return <Icons.Dna key={iconName} className="w-6 h-6" />
        case "Search":
          return <Icons.Search key={iconName} className="w-6 h-6" />
        case "TestTube":
          return <Icons.TestTube key={iconName} className="w-6 h-6" />
        case "Microscope":
          return <Icons.Microscope key={iconName} className="w-6 h-6" />
        default:
          return null
      }
    }) || []
  ...
  return (
    <div className="container mx-auto px-4 pt-32 pb-16">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-center mb-12">
          <Link href="/analisis" className="text-blue-600 hover:text-blue-800 flex items-center gap-2">
